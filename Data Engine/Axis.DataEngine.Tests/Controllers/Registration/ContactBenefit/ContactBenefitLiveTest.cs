...
namespace Axis.DataEngine.Tests.Controllers.Registration.ContactBenefit
{
    ...
    public class ContactBenefitLiveTest
    {
        ...
        public void GetPayorPlans_Failure()
        {
            ...
            Assert.IsTrue(response.DataItems.Count == 0, "Response contains data items for invalid data");
        }
        ...
        {
            // Arrange
            var url = baseRoute + "GetGroupsAndAddressForPlan";
            var param = new NameValueCollection();
            int planId = 1;
            ...
            Assert.IsTrue(response.DataItems[0].PayorAddresses.Count > 0, "PayorAddresses exists.");
            Assert.IsNotNull(response.DataItems[0].PlanGroups, "Groups does not exist for plan.");
            Assert.IsTrue(response.DataItems[0].PlanGroups.Count > 0, "Group exists.");
        }
        ...
        //    var param = new NameValueCollection();
        //    int contactId = -1;
        //    param.Add("Id", contactId.ToString());
