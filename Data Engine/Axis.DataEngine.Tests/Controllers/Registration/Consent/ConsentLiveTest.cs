using System;
using System.Collections.Specialized;
using System.Configuration;
using System.Net.Http;
using System.Net.Http.Formatting;
using System.Runtime.InteropServices;
using System.Threading.Tasks;
using System.Web.Helpers;
using Axis.Model.Common;
using Axis.Model.Registration;
using Axis.Service;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace Axis.DataEngine.Tests.Controllers.Registration.Consent
{
    [TestClass]
    public class ConsentLiveTest : BaseLiveTestController
    {
        #region Class Variables

        private long _defaultContactId = 1;
        private string _sigImage = "";

        #endregion

        #region Test Methods

        [TestMethod]
        public async Task AddConsentSignature_Success()
        {
            var addModel = new ConsentModel() { SignatureBlob = Convert.FromBase64String(_sigImage), ContactId = _defaultContactId, IsActive = true, ModifiedBy = 3, ForceRollback = true };

            var response =
                await _httpClient.PostAsync<ConsentModel>("/api/Consent/AddConsentSignature", addModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(responseModel.RowAffected > 0, "Consent signature could not be saved.");
        }

        [TestMethod]
        public async Task AddConsentSignature_Failure()
        {
            var addModel = new ConsentModel() { SignatureBlob = Convert.FromBase64String(_sigImage), ContactId = -1, IsActive = true, ModifiedBy = 3, ForceRollback = true };

            var response =
                await _httpClient.PostAsync<ConsentModel>("/api/Consent/AddConsentSignature", addModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(responseModel.ResultCode != 0, "Consent signature saved, but it should have failed.");
        }

        [TestMethod]
        public async Task GetConsentSignature_Success()
        {
            var response = await _httpClient.GetAsync(string.Format("/api/Consent/GetConsentSignature?contactId={0}", _defaultContactId.ToString()));
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.DataItems != null, "Data items cannot be null");
            Assert.IsTrue(model.DataItems.Count > 0, "At least one signature must exist for this contact");
        }

        [TestMethod]
        public async Task GetConsentSignature_Failure()
        {
            var response = await _httpClient.GetAsync(string.Format("/api/Consent/GetConsentSignature?contactId={0}", "-1"));
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.DataItems != null, "Data items cannot be null");
            Assert.IsTrue(model.ResultCode != 0, "This request succeeded, but it should have failed");
        }

        #endregion
    }
}
