...
namespace Axis.DataEngine.Tests.Controllers
{
    ...
    /// </summary>
    [TestClass]
    public class AdminControllerLiveTest : BaseLiveTestController
    {
        #region Test Methods
        ...
        {
            var response = await _httpClient.GetAsync("/api/Admin/GetUsers?firstName=&lastName=&userName=&isActive=true");
        ...
        public async Task AddUser()
        {
            ...
            var response =
                await _httpClient.PostAsync<UserModel>("/api/Admin/AddUser", addModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
        ...
        public async Task UpdateUser()
        {
            ...
            var response =
                await
                    _httpClient.PostAsync<UserModel>("/api/Admin/UpdateUser", updateModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<UserModel>>(result);
            Assert.IsTrue(responseModel.ResultCode == 0, "Result Code must be 0 (Success)");
            Assert.IsTrue(responseModel.RowAffected == 1, "Rows Affected must equal 1");
        ...
        public async Task RemoveUser()
        {
            ...
            var response =
                await
                    _httpClient.PostAsync<UserModel>("/api/Admin/RemoveUser", removeModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<UserModel>>(result);
            Assert.IsTrue(responseModel.ResultCode == 0, "Result Code must be 0 (Success)");
            Assert.IsTrue(responseModel.RowAffected == 1, "Rows Affected must equal 1");
        ...
        public async Task ActivateUser()
        {
            ...
            var response =
                await
                    _httpClient.PostAsync<UserModel>("/api/Admin/ActivateUser", activateModel,
        ...
        public async Task ResetUser()
        {
            ...
            var response =
                await _httpClient.PostAsync<UserModel>("/api/Admin/ResetUser", resetModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<UserModel>>(result);
            Assert.IsTrue(responseModel.ResultCode == 0, "Result Code must be 0 (Success)");
            Assert.IsTrue(responseModel.RowAffected == 1, "Rows Affected must equal 1");
        ...
        public async Task UnlockUser()
        {
            ...
            var response =
                await _httpClient.PostAsync<UserModel>("/api/Admin/UnlockUser", resetModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var responseModel = Json.Decode<Response<UserModel>>(result);
            Assert.IsTrue(responseModel.ResultCode == 0, "Result Code must be 0 (Success)");
            Assert.IsTrue(responseModel.RowAffected == 1, "Rows Affected must equal 1");
        ...
        {
            var response = await _httpClient.GetAsync("/api/Admin/GetUserRoles?userID=3");
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<UserRoleModel>>(result);
        ...
        {
            //Arrange
            var response = await _httpClient.GetAsync("/api/Admin/GetUserCredentials?userID=1");
            var result = response.Content.ReadAsStringAsync().Result;
