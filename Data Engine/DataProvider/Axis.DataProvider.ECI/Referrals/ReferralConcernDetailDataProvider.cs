...
namespace Axis.DataProvider.ECI.Referrals
{
    public class ReferralConcernDetailDataProvider : IReferralConcernDetailDataProvider
    {
        ...
        /// <param name="referralID">The referral identifier.</param>
        /// <returns></returns>
        public Response<ReferralConcernDetailModel> DeleteReferralConcernDetail(long referralConcernDetailID)
        {
            var referralRepository = unitOfWork.GetRepository<ReferralConcernDetailModel>(SchemaName.Registration);
            var procParams = new List<SqlParameter> { new SqlParameter("ReferralConcernDetailID", referralConcernDetailID) };
