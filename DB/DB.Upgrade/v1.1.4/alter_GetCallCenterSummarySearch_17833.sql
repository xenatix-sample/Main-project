GO
IF EXISTS(SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'CallCenter.usp_GetCallCenterSummarySearch'))
       BEGIN
       DROP PROCEDURE CallCenter.usp_GetCallCenterSummarySearch
       END
     ...
     BEGIN TRY
        ...
        IF ISNULL(@SearchCriteria,'') = ''
                     ...
                     SELECT
                           ...
                           (SELECT TOP 1 CLRPH.Number FROM Registration.ContactPhone CLRCPH
                           JOIN Core.Phone CLRPH
                                  ON CLRPH.PhoneID=CLRCPH.PhoneID AND CLRCPH.IsPrimary=1 where 
                           CLRCPH.ContactID = CLR.ContactID ) AS CallerContactNumber, 
                     ...
                     WHERE
                           ...
                           (
                                  ...
                                  (
                                         (
                                                ...
                                                AND CAST(CH.CallStartTime AS DATE) BETWEEN DATEADD(d, -3, CAST(CURRENT_TIMESTAMP AS DATE)) AND CAST(CURRENT_TIMESTAMP AS DATE)
                                         )
                                         OR
                                         (
                                                (CH.ProviderID = @UserIDFilter OR ISNULL(@UserIDFilter, 0) = 0)
                           ...
                           ORDER BY CH.CallStartTime DESC
                     END
              ELSE
                     BEGIN
