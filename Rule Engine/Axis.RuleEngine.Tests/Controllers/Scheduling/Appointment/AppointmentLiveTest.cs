...
namespace Axis.DataEngine.Tests.Controllers.Scheduling.Appointment
{
    ...
    public class AppointmentLiveTest
    {
        ...
        /// <summary>
        /// Failure test case for GetAppointmentDetail
        /// </summary>
        [TestMethod]
        public void GetAppointmentDetail_Failed()
        {
            // Arrange
            var url = baseRoute + "getAppointmentDetail";

            var param = new NameValueCollection();
            param.Add("contactId", "-1");

            // Act
            var response = communicationManager.Get<Response<AppointmentDetailModel>>(param, url);
        ...
        public void CancelAppointment_Failed()
        {
            ...
            // Assert
            Assert.IsTrue(response != null, "Response can't be null");
            Assert.IsTrue(response.ResultCode != 0, "Cancelled appointment expected to be failed.");
        }
    }
}