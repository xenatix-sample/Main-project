using System;
using System.Collections.Specialized;
using System.Configuration;
using System.Net.Http;
using System.Net.Http.Formatting;
using System.Threading.Tasks;
using System.Web.Helpers;
using Axis.Model.Common;
using Axis.Model.Registration;
using Axis.Service;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace Axis.RuleEngine.Tests.Controllers.Registration.Consent
{
    [TestClass]
    public class ConsentLiveTest
    {
        #region Class Variables

        //private CommunicationManager _communicationManager;
        private const string BaseRoute = "Consent/";
        private long _defaultContactId = 1;
        private string _sigImage = "";
        private byte[] _convertedImage;
        private HttpClient httpClient;

        #endregion

        #region Test Methods

        [TestInitialize]
        public void Initialize()
        {
            _convertedImage = Convert.FromBase64String(_sigImage);

            httpClient = new HttpClient();
            httpClient.BaseAddress = new Uri(ConfigurationManager.AppSettings["UnitTestUrl"]);
            httpClient.DefaultRequestHeaders.Add("X-Token", ConfigurationManager.AppSettings["UnitTestToken"]);
        }

        /// <summary>
        /// Tears down.
        /// </summary>
        [TestCleanup]
        public void TearDown()
        {
            httpClient.Dispose();
        }

        [TestMethod]
        public async Task AddConsentSignature_Success()
        {
            var addModel = new ConsentModel() { SignatureBlob = _convertedImage, ContactId = _defaultContactId, IsActive = true, ModifiedBy = 3, ForceRollback = true };

            var response =
               await httpClient.PostAsync<ConsentModel>("/api/Consent/AddConsentSignature", addModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.RowAffected > 0, "Consent signature could not be saved.");
        }

        [TestMethod]
        public async Task AddConsentSignature_Failure()
        {
            var addModel = new ConsentModel() { SignatureBlob = _convertedImage, ContactId = -1, IsActive = true, ModifiedBy = 3, ForceRollback = true };

            var response =
               await httpClient.PostAsync<ConsentModel>("/api/Consent/AddConsentSignature", addModel, new JsonMediaTypeFormatter());
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.ResultCode != 0, "Consent signature saved, but it should have failed.");
        }

        [TestMethod]
        public async Task GetConsentSignature_Success()
        {
            var response = await httpClient.GetAsync(string.Format("/api/Consent/GetConsentSignature?contactId={0}", _defaultContactId.ToString()));
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.DataItems.Count > 0);
        }

        [TestMethod]
        public async Task GetConsentSignature_Failure()
        {
            var response = await httpClient.GetAsync(string.Format("/api/Consent/GetConsentSignature?contactId={0}", "-1"));
            var result = response.Content.ReadAsStringAsync().Result;
            var model = Json.Decode<Response<ConsentModel>>(result);

            Assert.IsTrue(model.DataItems != null, "Data items cannot be null");
            Assert.IsTrue(model.ResultCode != 0, "This request succeeded, but it should have failed");
        }

        #endregion
    }
}
