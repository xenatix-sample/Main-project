...
var offCanvasNav = {
    init: function () {
        ...
        $('[data-toggle=offcanvason]').on('click', function () {
            sq.log('offCanvasNavOn click');
            if (!$('.row-offcanvas').hasClass('active')) {
                $('.row-offcanvas').addClass('active');
            }
        });

        $('[data-toggle=offcanvasoff]').on('click', function () {
            sq.log('offCanvasNavOff click');
            if ($('.row-offcanvas').hasClass('active')) {
    ...
    calcOffCanvasHeight : function(){
        var mainContainer = $(document).height() - $('.navbar-fixed-top').height();
        var panelContainer = mainContainer - 60;
        $('.sidebar-offcanvas').css({ 'min-height': mainContainer }); // min-height makes this nicer for mobile
        $('.sidebar-offcanvas .section-block').css({ 'min-height': panelContainer });
        $('.sidebar-offcanvas .profile_img').css({ 'min-height': panelContainer });
    }
};
...
var sq = {
	...
	search: {
		debug: false,
		searchToolA: '.search-form-tool a',
		searchFormWrap: '.search-form-tool .search-form',
		slider: '.search-form-tool .search-form .slider',

		init: function(){
			jQuery(sq.search.searchToolA).click( function(){
				
				jQuery(sq.search.slider).toggleClass('active');
			} );
		}
	},
	jsCenterContent: {
...
$(document).ready(function ($) {
    ...
    /* top nav mobile flyout */
	$("#navbar .nav a:not('.dropdown-toggle'), #navbar .nav button").on('click', function () {
