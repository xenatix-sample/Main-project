angular.module('xenatixApp.settings', [])
        .constant('settings', {
            "webApiBaseUrl": "http://localhost:5050"
        });
describe('registrationController', function () {
    var scope, registrationServiceMock, q, alertServiceMock;
    var obj = {
        DataItems: [{
            test: 123,
            DOB: '/DATE(1437737993)/',
            FullCodeDNR: false,
            MailPermission: true,
            ContactID: 1,
            Addresses: {
                city: 'delhi',
                country: 'India'
            }
        }]
    };
    var id = 20;
    beforeEach(function () {
        module('xenatixApp');
    });
    beforeEach(inject(function (_$rootScope_, _$controller_, _$q_, registrationService, alertService, $injector) {
        // create a scope object for us to use.
        $controller = _$controller_;
        $rootScope = _$rootScope_;
        scope = $rootScope.$new();
        q = _$q_;
        alertServiceMock = alertService;
        registrationServiceMock = registrationService;
        $httpBackend = $injector.get('$httpBackend');
        //mock Services
        var getMasterDeferred = q.defer();
        var getDeferred = q.defer();
        var addDeferred = q.defer();
        var editDeferred = q.defer();
        var removeDeferred = q.defer();
        $httpBackend.whenGET(/.*/).respond('login');
        spyOn(registrationServiceMock, 'getContactMasterData').andReturn(getMasterDeferred.promise);
        getMasterDeferred.resolve(obj);
        spyOn(registrationServiceMock, 'get').andReturn(getDeferred.promise);
        getDeferred.resolve(obj);
        spyOn(registrationServiceMock, 'add').andReturn(addDeferred.promise);
        addDeferred.resolve({ ID: id });
        spyOn(registrationServiceMock, 'update').andReturn(editDeferred.promise);
        editDeferred.resolve({ ID: id });
        spyOn(registrationServiceMock, 'remove').andReturn(removeDeferred.promise);
        removeDeferred.resolve(obj);
        spyOn(alertServiceMock, 'success').andReturn('success');
        spyOn(window, 'confirm').andReturn(true);
        // Declare controller with mocked services
        $controller('RegistrationController', {
            $scope: scope,
            registrationService: registrationServiceMock,
            alertService: alertServiceMock,
        });

    }));
    it('Get Contact', function () {
        scope.get(1);
        scope.$apply();
        expect(scope.newDemography).toEqual(jasmine.any(Object));
        expect(Object.keys(scope.newDemography).length).toBeGreaterThan(0);
    });
    it('Save Contact', function () {
        scope.newDemography = obj;
        scope.save(false, false, false);
        scope.$apply();
        expect(scope.newDemography.ContactID).toEqual(id);
    });
    it('Edit Contact', function () {
        scope.newDemography = obj;
        scope.edit();
        scope.$apply();
        expect(scope.newDemography.ContactID).toEqual(id);
    });
    it("Calculate Age from DOB", function () {
        scope.newDemography.DOB = 'Tue May 01 2001 00:00:00 GMT+0530 (India Standard Time)';
        scope.calculateAge();
        expect(scope.newDemography.Age).toEqual(14);
    });
    it('Cancel', function () {
        scope.newDemography = obj;
        scope.cancel();
        var check = { Phones: [], MailPermission: '0', FullCodeDNR: '0', IsPregnant: '0' };
        expect(scope.newDemography).toEqual(check);
    });
    it('Remove', function () {
        scope.newDemography = obj;
        scope.remove(obj);
        expect(scope.newDemography).toEqual(obj);
    });
});