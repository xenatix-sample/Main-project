...
using Axis.Helpers.ComponentModel;
    ...
    public class PluginManager
    {
        ...
        public static void Initialize()
        {
            using (new WriteLockDisposable(Locker))
            {
                ...
                _clearShadowDirectoryOnStartup = !String.IsNullOrEmpty(ApplicationSettings.ClearPluginsShadowDirectoryOnStartup) &&
                   Convert.ToBoolean(ApplicationSettings.ClearPluginsShadowDirectoryOnStartup);
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(AxisVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
            if (plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name +
                                                    " file exists in a folder outside of the allowed Axis folder heirarchy");
